...
//      Future(Redirect(controllers.docUpload.routes.SupportingDocController.onLoad().url))
//    ) { files =>
//      if (files.isEmpty) {
...
//  }
//
//  def onSubmit: Action[AnyContent] = requiresAtLeast(GatekeeperRole.USER) { implicit request =>
